var subroutines = {
	...
	withdrawEnergy: function(creep) {
		var containersWithEnergy = creep.room.find(FIND_STRUCTURES, {
			filter: function(i) {
				if (i.structureType == STRUCTURE_CONTAINER && i.store[RESOURCE_ENERGY] > 0) {
					return i;
				} else if (
		...
		//console.log('Found containers: ', containersWithEnergy);
	...
	deliverEnergy: function(creep) {
		//console.log('looking for needy creep');
		var needyCreep = creep.pos.findClosestByRange(FIND_MY_CREEPS, {
			filter: (c) => {
				return c.memory.role == ('upgrader' || 'builder' || 'repairer') && _.sum(c.carry) < c.carryCapacity;
			}
		});
		//console.log('Found: ', needyCreep);
		if (creep.transfer(needyCreep, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
			this.moveToMinCPU(creep, needyCreep);
		}
